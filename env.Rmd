---
title: "Envases Venezolanos (Reporte Grupo Interaval)"
...
<p align="center">
  <img width="380px" height="90px" src="env.png">
</p>

Envases Venezolanos, S.A., es una compañía que pertenece a un conglomerado de empresas dedicadas a la manufactura de empaques. Este Grupo está conformado por Envases Corona, Vidrios Domésticos (VIDOSA) y Tapas Corona. Envases Venezolanos es una empresa que cotiza en la Bolsa de Valores de Caracas bajo el símbolo "ENV" y hasta la fecha mantiene 126.923.808 acciones en circulación.
...
library(kableExtra)
  ...
  datos_accion <-function(accion, directorio, nombre){
    ...
    colnames(accion)<-c("Fecha","Tipo.de.operacion",paste(nombre,".Open",sep=""),paste(nombre,".Close",sep=""),
                        ...
                        ".Cantidad.de.Operaciones",paste(nombre,".Volume",sep=""),".Monto.ENVctivo")
    ...
    accion=accion[c("Fecha", paste(nombre,".Open",sep=""), paste(nombre,".High",sep=""), 
                    ...
                    ".Cantidad.de.Operaciones", ".Monto.ENVctivo", "Tipo.de.operacion") ]
    ##REVISADO
  ...
  mercado=data.frame(
    ...
    numero_de_acciones=c(126923808),
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( ENV$precio, mercado[mercado$`Acción`=="ENV",4], min(ENV$accion_usd$ENV.Close), max(ENV$accion_usd$ENV.Close), mean(ENV$accion_usd$ENV.Close),
        as.numeric(var(ENV$accion_usd$ENV.Close)), mercado[mercado$`Acción`=="ENV",6], mercado[mercado$`Acción`=="ENV",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(ENV$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(ENV$accion_bs)
