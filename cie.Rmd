---
title: "Corporación Industrial de Energía (Reporte Grupo Interaval)"
...
Se constituye en Septiembre de 1.998, con el objeto principal de efectuar operaciones de producción, generacion y prestación de servicios de energia, las cuales ejecuta a traves de sus compañías filiales.
...
library(kableExtra)
  ...
  mercado=data.frame(
    ...
    numero_de_acciones=c(485560500),
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( CIE$precio, mercado[mercado$`Acción`=="CIE",4], min(CIE$accion_usd$CIE.Close), max(CIE$accion_usd$CIE.Close), mean(CIE$accion_usd$CIE.Close),
        as.numeric(var(CIE$accion_usd$CIE.Close)), mercado[mercado$`Acción`=="CIE",6], mercado[mercado$`Acción`=="CIE",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(CIE$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(CIE$accion_bs)
