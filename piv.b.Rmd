---
title: " PIVCA Promotora de Inversiones y Valores (Reporte Grupo Interaval)"
...
<p align="center">
  <img width="300px" height="90px" src="pivb.png">
</p>

Su actividad es la captación de recursos para su inversión en empresas, con preferencia en las pequeñas y medianas (PYMES), que desarrollen su actividad en diferentes sectores económicos, con énfasis en el sector primario de la economía, a saber, agricultura, ganadería, silvicultura, pesca, entre otros. También, en el sector primario,secundario y terciario de la economia relacionada agroindustriay comercio exterior.
...
library(kableExtra)
  ...
  mercado=data.frame(
    ...
    numero_de_acciones=c(8100),
    
    
    precio=c(pivb$precio)
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( pivb$precio, mercado[mercado$`Acción`=="PIV.B",4], min(pivb$accion_usd$PIV.B.Close), max(pivb$accion_usd$PIV.B.Close), mean(pivb$accion_usd$PIV.B.Close),
        as.numeric(var(pivb$accion_usd$PIV.B.Close)), mercado[mercado$`Acción`=="PIV.B",6], mercado[mercado$`Acción`=="PIV.B",2])
...
accion=datos_accion(accion,paste("C:/Users/analista04/Documents/R/grupointeraval.github.io/","pivb.txt",sep=""),"PIV.B")
accion$grafico_usd
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(pivb$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(pivb$accion_bs)
