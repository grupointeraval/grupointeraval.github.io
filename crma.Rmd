---
title: "Corimon (Reporte Grupo Interaval)"
...
<p align="center">
  <img width="200px" height="100px" src="crma.jpg">
</p>

Es una corporación venezolana fundada en 1949 dedicada a la producción y comercialización de una gran diversidad de productos industriales relacionados al mundo de las pinturas, productos químicos y envases flexibles, contribuyendo al desarrollo nacional con conciencia y responsabilidad hacia el medio ambiente. Es una empresa que cotiza  en la Bolsa de Valores de Caracas bajo el símbolo "CRM.A" y hasta la fecha mantiene 150.000.000 acciones en circulación.
...
library(kableExtra)
  ...
  CRM.A=datos_accion(accion,paste("C:/Users/analista04/Documents/R/grupointeraval.github.io//","CRM.A.txt",sep=""),"CRM.A")
  ...
  mercado=data.frame(
    ...
    numero_de_acciones=c(150000000),
    
    
    precio=c(CRM.A$precio)
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( CRM.A$precio, mercado[mercado$`Acción`=="CRM.A",4], min(CRM.A$accion_usd$CRM.A.Close), max(CRM.A$accion_usd$CRM.A.Close), mean(CRM.A$accion_usd$CRM.A.Close),
        as.numeric(var(CRM.A$accion_usd$CRM.A.Close)), mercado[mercado$`Acción`=="CRM.A",6], mercado[mercado$`Acción`=="CRM.A",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(CRM.A$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(CRM.A$accion_bs)
