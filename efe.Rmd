---
title: "Productos EFE (Reporte Grupo Interaval)"
...
<p align="center">
  <img width="200px" height="100px" src="efe.jpg">
</p>

Esta compañía fue fundada en Caracas el 8 de mayo de 1926, por Alberto Espinoza Blanco y su esposa Mila Fernández de Espinoza. De la fusión de las iniciales de sus apellidos surge la marca EFE (Espinoza-Fernández), quienes producían helados caseros en la Parroquia La Candelaria de Caracas. Actualmente forma parte de Alimentos Polar y cotiza en la Bolsa de Valores de Caracas bajo el símbolo "EFE" y hasta la fecha mantiene 700.000.000 acciones en circulación.
...
library(kableExtra)
  ...
  mercado=data.frame(
    ...
    numero_de_acciones=c(700000000),
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( EFE$precio, mercado[mercado$`Acción`=="EFE",4], min(EFE$accion_usd$EFE.Close), max(EFE$accion_usd$EFE.Close), mean(EFE$accion_usd$EFE.Close),
        as.numeric(var(EFE$accion_usd$EFE.Close)), mercado[mercado$`Acción`=="EFE",6], mercado[mercado$`Acción`=="EFE",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(EFE$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(EFE$accion_bs)
