---
title: "Mercantil Servicios Financieros A (Reporte Grupo Interaval)"
...
Es una  empresa venezolana de servicios financieros que en el transcurso de su historia ha acumulado experiencia en 9 países de América y Europa. La coMVZ.Añía tiene por objeto la realización de toda clase de inversiones en acciones, bonos, cuotas de participación y demás obligaciones provenientes de entidades mercantiles; la promoción de coMVZ.Añías de comercio en todas sus alternativas y modalidades; actuar como agente mediador de comercio y como comisionista en operaciones de promoción e inversión financiera. Esta empresa cotiza en la Bolsa de Valores de Caracas bajo los símbolos "MVZ.A" y "MVZ.B". Hasta la fecha mantiene 104.760.961 acciones en circulación.
...
library(kableExtra)
  ...
  datos_accion <-function(accion, directorio, nombre){
    ...
    colnames(accion)<-c("Fecha","Tipo.de.operacion",paste(nombre,".Open",sep=""),paste(nombre,".Close",sep=""),
                        ...
                        ".Cantidad.de.Operaciones",paste(nombre,".Volume",sep=""),".Monto.MVZ.Activo")
    ...
    accion=accion[c("Fecha", paste(nombre,".Open",sep=""), paste(nombre,".High",sep=""), 
                    ...
                    ".Cantidad.de.Operaciones", ".Monto.MVZ.Activo", "Tipo.de.operacion") ]
    ##REVISADO
  ...
  MVZ.A=datos_accion(accion,paste("C:/Users/analista04/Documents/R/grupointeraval.github.io//","MVZ.A.txt",sep=""),"MVZ.A")
  ...
  mercado=data.frame(
    ...
    numero_de_acciones=c(60880929),
    
    
    precio=c(MVZ.A$precio)
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( MVZ.A$precio, mercado[mercado$`Acción`=="MVZ.A",4], min(MVZ.A$accion_usd$MVZ.A.Close), max(MVZ.A$accion_usd$MVZ.A.Close), mean(MVZ.A$accion_usd$MVZ.A.Close),
        as.numeric(var(MVZ.A$accion_usd$MVZ.A.Close)),  mercado[mercado$`Acción`=="MVZ.A",6], mercado[mercado$`Acción`=="MVZ.A",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(MVZ.A$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(MVZ.A$accion_bs)
