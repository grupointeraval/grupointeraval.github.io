---
title: "Fábrica Nacional de Cementos (Reporte Grupo Interaval)"
...
<p align="center">
  <img width="80px" height="90px" src="fnc.jpg">
</p>

La Fábrica Nacional de Cementos S.A.C.A (FNC) es una empresa comprometida con la producción y comercialización de cemento, material estratégico para la construcción de la más alta calidad. Produce concreto premezclado con criterios de eficiencia, calidad, confiabilidad y sustentabilidad. Esta empresa cotiza en la Bolsa de Valores de Caracas bajo el símbolo "FNC" y hasta la fecha mantiene 81.081.000 acciones en circulación.
...
library(kableExtra)
  ...
  datos_accion <-function(accion, directorio, nombre){
    ...
    colnames(accion)<-c("Fecha","Tipo.de.operacion",paste(nombre,".Open",sep=""),paste(nombre,".Close",sep=""),
                        ...
                        ".Cantidad.de.Operaciones",paste(nombre,".Volume",sep=""),".Monto.FNCctivo")
    ...
    accion=accion[c("Fecha", paste(nombre,".Open",sep=""), paste(nombre,".High",sep=""), 
                    ...
                    ".Cantidad.de.Operaciones", ".Monto.FNCctivo", "Tipo.de.operacion") ]
    ##REVISADO
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( FNC$precio, mercado[mercado$`Acción`=="FNC",4], min(FNC$accion_usd$FNC.Close), max(FNC$accion_usd$FNC.Close), mean(FNC$accion_usd$FNC.Close),
        as.numeric(var(FNC$accion_usd$FNC.Close)), mercado[mercado$`Acción`=="FNC",6], mercado[mercado$`Acción`=="FNC",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(FNC$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(FNC$accion_bs)
