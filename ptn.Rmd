---
title: "Protinal (Reporte Grupo Interaval)"
...
<p align="center">
  <img width="310px" height="90px" src="ptn.jpg">
</p>

Es una empresa con 77 años de trayectoria en el mercado venezolano a la vanguardia dentro del sector de producción de alimentos en el país. A través de sus marcas: Del Corral®, Protinal® y Protican® cuentan con un excelente posicionamiento siendo una pieza clave en la nutrición del país a través de todo su abanico de productos, contando además con una fortaleza única en distribución, estando presente en casi todo el territorio con 10 centros de distribución. La actividad principal de esta sociedad es la elaboración de productos alimenticios para animales y procesamiento de productos agrícolas. Esta empresa cotiza en la Bolsa de Valores de Caracas bajo los símbolos "PTN" y "PGR" . Hasta la fecha mantiene 885.304.420 acciones en circulación.
...
library(kableExtra)
  ...
  datos_accion <-function(accion, directorio, nombre){
    ...
    colnames(accion)<-c("Fecha","Tipo.de.operacion",paste(nombre,".Open",sep=""),paste(nombre,".Close",sep=""),
                        ...
                        ".Cantidad.de.Operaciones",paste(nombre,".Volume",sep=""),".Monto.PTNctivo")
    ...
    accion=accion[c("Fecha", paste(nombre,".Open",sep=""), paste(nombre,".High",sep=""), 
                    ...
                    ".Cantidad.de.Operaciones", ".Monto.PTNctivo", "Tipo.de.operacion") ]
    ##REVISADO
  ...
  mercado=data.frame(
    ...
    numero_de_acciones=c(152986904),
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( PTN$precio, mercado[mercado$`Acción`=="PTN",4], min(PTN$accion_usd$PTN.Close), max(PTN$accion_usd$PTN.Close), mean(PTN$accion_usd$PTN.Close),
        as.numeric(var(PTN$accion_usd$PTN.Close)),  mercado[mercado$`Acción`=="PTN",6], mercado[mercado$`Acción`=="PTN",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(PTN$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(PTN$accion_bs)
