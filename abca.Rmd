---
title: "Banco del Caribe (Reporte Grupo Interaval)"
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( abca$precio, mercado[mercado$`Acción`=="ABC.A",4], min(abca$accion_usd$ABC.A.Close), max(abca$accion_usd$ABC.A.Close), mean(abca$accion_usd$ABC.A.Close),
        as.numeric(var(abca$accion_usd$ABC.A.Close)), mercado[mercado$`Acción`=="ABC.A",6], mercado[mercado$`Acción`=="ABC.A",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(abca$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(abca$accion_bs)
