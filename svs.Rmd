---
title: "SIVENSA (Reporte Grupo Interaval)"
...
<p align="center">
  <img width="200px" height="90px" src="svs.jpg">
</p>

Esta empresa desarrolla actividades siderúrgicas y metalúrgicas para el mercado nacional y de exportación, a través de sus divisiones: manufactura productos de acero semiterminados y terminados para la industria laminadora y construcción respectivamente; briquetas de hierro para el sector siderúrgico y alambres para uso industrial y agroindustrial. Esta compañía cotiza en la Bolsa de Valores de Caracas bajo el símbolo "SVS" y hasta la fecha mantiene 52.524.376 acciones en circulación.
...
library(kableExtra)
  ...
  datos_accion <-function(accion, directorio, nombre){
    ...
    colnames(accion)<-c("Fecha","Tipo.de.operacion",paste(nombre,".Open",sep=""),paste(nombre,".Close",sep=""),
                        ...
                        ".Cantidad.de.Operaciones",paste(nombre,".Volume",sep=""),".Monto.SVSctivo")
    ...
    accion=accion[c("Fecha", paste(nombre,".Open",sep=""), paste(nombre,".High",sep=""), 
                    ...
                    ".Cantidad.de.Operaciones", ".Monto.SVSctivo", "Tipo.de.operacion") ]
    ##REVISADO
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( SVS$precio, mercado[mercado$`Acción`=="SVS",4], min(SVS$accion_usd$SVS.Close), max(SVS$accion_usd$SVS.Close), mean(SVS$accion_usd$SVS.Close),
        as.numeric(var(SVS$accion_usd$SVS.Close)), mercado[mercado$`Acción`=="SVS",6], mercado[mercado$`Acción`=="SVS",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(SVS$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(SVS$accion_bs)
