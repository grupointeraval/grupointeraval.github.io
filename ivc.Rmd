---
title: "INVACA (Reporte Grupo Interaval)"
...
<p align="center">
  <img width="350px" height="90px" src="ivc.png">
</p>

Esta sociedad tiene por objeto la adquisición y enajenación por cualquier medio, asi como la explotaciÓn de bienes inmuebles y titulos valores. Diseño, ejecución y administración de proyectos inmobiliarios y de inversión. Promocion y constitución de toda clase de sociedades mercantiles dentro ofuera del territorio nacional. Celebración de toda clase de contratos civiles, mercantiles y administrativos.
...
library(kableExtra)
  ...
  datos_accion <-function(accion, directorio, nombre){
    ...
    colnames(accion)<-c("Fecha","Tipo.de.operacion",paste(nombre,".Open",sep=""),paste(nombre,".Close",sep=""),
                        ...
                        ".Cantidad.de.Operaciones",paste(nombre,".Volume",sep=""),".Monto.IVCctivo")
    ...
    accion=accion[c("Fecha", paste(nombre,".Open",sep=""), paste(nombre,".High",sep=""), 
                    ...
                    ".Cantidad.de.Operaciones", ".Monto.IVCctivo", "Tipo.de.operacion") ]
    ##REVISADO
  ...
  mercado=data.frame(
    ...
    numero_de_acciones=c(111025339),
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( IVC$precio, mercado[mercado$`Acción`=="IVC",4], min(IVC$accion_usd$IVC.Close), max(IVC$accion_usd$IVC.Close), mean(IVC$accion_usd$IVC.Close),
        as.numeric(var(IVC$accion_usd$IVC.Close)),  mercado[mercado$`Acción`=="IVC",6], mercado[mercado$`Acción`=="IVC",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(accion$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(accion$accion_bs)
