---
title: "Domínguez & CIA (Reporte Grupo Interaval)"
...
Es una empresa Fundada en 1930, dedicada a la elaboración de empaques de Hojalata, Aluminio y Plástico con el fin de atender al mercado nacional y exportación en los reglones de Alimentos y Bebidas, Industrial y farmacéutica. Cotiza en la Bolsa de Valores de Caracas bajo el símbolo "DOM" y hasta la fecha mantiene 24.062.500 acciones en circulación.
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( DOM$precio, mercado[mercado$`Acción`=="DOM",4], min(DOM$accion_usd$DOM.Close), max(DOM$accion_usd$DOM.Close), mean(DOM$accion_usd$DOM.Close),
        as.numeric(var(DOM$accion_usd$DOM.Close)), mercado[mercado$`Acción`=="DOM",6], mercado[mercado$`Acción`=="DOM",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(DOM$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(DOM$accion_bs)
