---
title: "Telares Palo Grande (Reporte Grupo Interaval)"
...
<p align="center">
  <img width="200px" height="200px" src="tpg.jpg">
</p>

Es una fábrica textil competitiva, de alta calidad, cercana a sus clientes y custodia de la marca número uno en textiles para el hogar venezolano: Ama de Casa®. Fue fundada en el año 1920 y desde entonces su negocio ha estado relacionado con la elaboración de telas, lencerías y toallas, para el uso doméstico e industrial. Es una empresa que cotiza  en la Bolsa de Valores de Caracas bajo el símbolo "TPG" y hasta la fecha mantiene 1.188.344.665 acciones en circulación.
...
library(kableExtra)
  ...
  datos_accion <-function(accion, directorio, nombre){
    ...
    colnames(accion)<-c("Fecha","Tipo.de.operacion",paste(nombre,".Open",sep=""),paste(nombre,".Close",sep=""),
                        ...
                        ".Cantidad.de.Operaciones",paste(nombre,".Volume",sep=""),".Monto.TPGctivo")
    ...
    accion=accion[c("Fecha", paste(nombre,".Open",sep=""), paste(nombre,".High",sep=""), 
                    ...
                    ".Cantidad.de.Operaciones", ".Monto.TPGctivo", "Tipo.de.operacion") ]
    ##REVISADO
  ...
  mercado=data.frame(
    ...
    numero_de_acciones=c(1188344665),
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( TPG$precio, mercado[mercado$`Acción`=="TPG",4], min(TPG$accion_usd$TPG.Close), max(TPG$accion_usd$TPG.Close), mean(TPG$accion_usd$TPG.Close),
        as.numeric(var(TPG$accion_usd$TPG.Close)),  mercado[mercado$`Acción`=="TPG",6], mercado[mercado$`Acción`=="TPG",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(TPG$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(TPG$accion_bs)
