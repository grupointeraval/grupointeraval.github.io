---
title: "Banco Occidental de Descuento (Reporte Grupo Interaval)"
...
<p align="center">
  <img width="100px" height="100px" src="bou.png">
</p>

El Banco Occidental de Descuento(B.O.D) es una institución financiera de capital venezolano y es uno de los bancos privados más grandes de ese país. Es representante en Venezuela de la marca American Express. Cuenta con 332 oficinas y más de 50 mil puntos de venta, para satisfacer la demanda de más de 4 millones de clientes en Venezuela. Esta empresa cotiza en la Bolsa de Valores de Caracas bajo el símbolo "BOU" y hasta lafecha mantiene 23.796.748.346 acciones en circulación.
...
library(kableExtra)
  ...
  mercado=data.frame(
    ...
    numero_de_acciones=c(23796748346),
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( BOU$precio, mercado[mercado$`Acción`=="BOU",4], min(BOU$accion_usd$BOU.Close), max(BOU$accion_usd$BOU.Close), mean(BOU$accion_usd$BOU.Close),
        as.numeric(var(BOU$accion_usd$BOU.Close)), mercado[mercado$`Acción`=="BOU",6], mercado[mercado$`Acción`=="BOU",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(BOU$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(BOU$accion_bs)
