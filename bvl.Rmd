---
title: "Banco de Venezuela (Reporte Grupo Interaval)"
...
<p align="center">
  <img width="500px" height="200px" src="bvl.jpg">
</p>

Es una institución financiera de banca universal con sede en Caracas. Fue fundada el 2 de septiembre de 1890, y se dedica a la actividad de la intermediación financiera, consistente en la captación de recursos con la finalidad de otorgar créditos o financiamientos y demás operaciones o servicios financieros. Esta empresa cotiza en la Bolsa de Valores de Caracas bajo el símbolo "BVL" y hasta la fecha mantiene 3.647.133.702 acciones en circulación.
...
library(kableExtra)
  ...
  mercado=data.frame(
    ...
    numero_de_acciones=c(3647133702),
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( BVL$precio, mercado[mercado$`Acción`=="BVL",4], min(BVL$accion_usd$BVL.Close), max(BVL$accion_usd$BVL.Close), mean(BVL$accion_usd$BVL.Close),
        as.numeric(var(BVL$accion_usd$BVL.Close)), mercado[mercado$`Acción`=="BVL",6], mercado[mercado$`Acción`=="BVL",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(BVL$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(BVL$accion_bs)
