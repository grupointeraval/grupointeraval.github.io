---
title: "Mercantil Servicios Financieros B (Reporte Grupo Interaval)"
...
<p align="center">
  <img width="200px" height="90px" src="mvz.png">
</p>

Es una  empresa venezolana de servicios financieros que en el transcurso de su historia ha acumulado experiencia en 9 países de América y Europa. La coMVZ.Bñía tiene por objeto la realización de toda clase de inversiones en acciones, bonos, cuotas de participación y demás obligaciones provenientes de entidades mercantiles; la promoción de coMVZ.Bñías de comercio en todas sus alternativas y modalidades; actuar como agente mediador de comercio y como comisionista en operaciones de promoción e inversión financiera. Esta empresa cotiza en la Bolsa de Valores de Caracas bajo los símbolos "MVZ.A" y "MVZ.B". Hasta la fecha mantiene 104.760.961 acciones en circulación.
...
library(kableExtra)
  ...
  datos_accion <-function(accion, directorio, nombre){
    ...
    colnames(accion)<-c("Fecha","Tipo.de.operacion",paste(nombre,".Open",sep=""),paste(nombre,".Close",sep=""),
                        ...
                        ".Cantidad.de.Operaciones",paste(nombre,".Volume",sep=""),".Monto.MVZ.Bctivo")
    ...
    accion=accion[c("Fecha", paste(nombre,".Open",sep=""), paste(nombre,".High",sep=""), 
                    ...
                    ".Cantidad.de.Operaciones", ".Monto.MVZ.Bctivo", "Tipo.de.operacion") ]
    ##REVISADO
  ...
  MVZ.B=datos_accion(accion,paste("C:/Users/analista04/Documents/R/grupointeraval.github.io//","MVZ.B.txt",sep=""),"MVZ.B")
  ...
  mercado=data.frame(
    ...
    numero_de_acciones=c(43880032),
    
    
    precio=c(MVZ.B$precio)
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    dato = c ("Precio Bs.","Precio $USD", "Mínimo $USD","Máximo $USD","Media $USD","Varianza $USD",  "Capitalización $USD", "Cantidad de acciones"),
    
    valor=c( MVZ.B$precio, mercado[mercado$`Acción`=="MVZ.B",4], min(MVZ.B$accion_usd$MVZ.B.Close), max(MVZ.B$accion_usd$MVZ.B.Close), mean(MVZ.B$accion_usd$MVZ.B.Close),
        as.numeric(var(MVZ.B$accion_usd$MVZ.B.Close)),  mercado[mercado$`Acción`=="MVZ.B",6], mercado[mercado$`Acción`=="MVZ.B",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(MVZ.B$accion_usd)
excel[,4] <- NULL
colnames(excel)=c("Apertura", "Cierre", "Mínimo", "Máximo")
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(MVZ.B$accion_bs)
