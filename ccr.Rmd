---
title: "Cerámicas Carabobo (Reporte Grupo Interaval)"
...
<p align="center">
  <img width="365px" height="80px" src="ccr.png">
</p>

Empresa manufacturera. Tiene por objeto la producción y comercialización nacional e internacional de revestimientos cerámicos para pisos y paredes, además de la elaboración de productos refractarios resistentes a altas temperaturas para la industria.
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( CCR$precio, mercado[mercado$`Acción`=="CCR",4], min(CCR$accion_usd$CCR.Close), max(CCR$accion_usd$CCR.Close), mean(CCR$accion_usd$CCR.Close),
        as.numeric(var(CCR$accion_usd$CCR.Close)), mercado[mercado$`Acción`=="CCR",6], mercado[mercado$`Acción`=="CCR",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(CCR$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(CCR$accion_bs)
