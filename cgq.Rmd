---
title: "Corporación Grupo Químico (Reporte Grupo Interaval)"
...
<p align="center">
  <img width="200px" height="200px" src="cgq.jpg">
</p>

Es una Compañia Matriz de diversas operadoras que desarrollan actividades en diferentes ramas de la industria de la pintura y quimicos, elaborando infinidad de productos dentro de los cuales encontramos pinturas domesticas e industrial, pintura automotriz, barnices y resinas etc.
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( CGQ$precio, mercado[mercado$`Acción`=="CGQ",4], min(CGQ$accion_usd$CGQ.Close), max(CGQ$accion_usd$CGQ.Close), mean(CGQ$accion_usd$CGQ.Close),
        as.numeric(var(CGQ$accion_usd$CGQ.Close)), mercado[mercado$`Acción`=="CGQ",6], mercado[mercado$`Acción`=="CGQ",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(CGQ$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(CGQ$accion_bs)
