...
DT::datatable(mercado, class = 'cell-border stripe', extensions="Buttons", options = list(pageLength = 27, dom = 'Bfrtip', buttons = c('excel', 'pdf', 'print')))
...
En estadística el coeficiente de correlación de Pearson como un índice que puede utilizarse para medir el grado de relación de dos variables, a medida que valor de este coeficiente se acerca 1 el grado de relación es positivo entre las variables, en este caso se entendería que si una accion incremente su valor, la otra lo hará en cierta proporción. En el caso opuesto, a medida que el coeficiente se acerca a -1, el grado de relación es negativo, interpretando en este caso que si una accio sube, la otra baja. A continuación se dispone de un mapa de correlación que muestra los distintos grados de relación entre las acciones que cotizan en la Bolsa de Valores de Caracas. Para la construcción de este gráfico se utilizó la serie anaual más reciente de cada una de las acciones, descartando el título RST.B. 
