---
title: "Manufacturas de Papel (Reporte Grupo Interaval)"
...
<p align="center">
  <img width="100px" height="100px" src="MPA.png">
</p>

Empresa papelera. Su objeto es la producción de pulpa, papeles, cartulinas, recolección de fibras reciclables, bolsas de papel, sacos multipliegos, papel higiénico, toallas faciales y servilletas; además de la elaboración de productos escolares.
...
library(kableExtra)
  ...
  datos_accion <-function(accion, directorio, nombre){
    ...
    colnames(accion)<-c("Fecha","Tipo.de.operacion",paste(nombre,".Open",sep=""),paste(nombre,".Close",sep=""),
                        ...
                        ".Cantidad.de.Operaciones",paste(nombre,".Volume",sep=""),".Monto.MPActivo")
    ...
    accion=accion[c("Fecha", paste(nombre,".Open",sep=""), paste(nombre,".High",sep=""), 
                    ...
                    ".Cantidad.de.Operaciones", ".Monto.MPActivo", "Tipo.de.operacion") ]
    ##REVISADO
  ...
  mercado=data.frame(
    ...
    numero_de_acciones=c(229410000),
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( MPA$precio, mercado[mercado$`Acción`=="MPA",4], min(MPA$accion_usd$MPA.Close), max(MPA$accion_usd$MPA.Close), mean(MPA$accion_usd$MPA.Close),
        as.numeric(var(MPA$accion_usd$MPA.Close)),  mercado[mercado$`Acción`=="MPA",6], mercado[mercado$`Acción`=="MPA",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(MPA$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(MPA$accion_bs)
