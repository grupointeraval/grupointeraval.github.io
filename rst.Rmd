...
library(kableExtra)
  ...
  datos_accion <-function(accion, directorio, nombre){
    ...
    colnames(accion)<-c("Fecha","Tipo.de.operacion",paste(nombre,".Open",sep=""),paste(nombre,".Close",sep=""),
                        ...
                        ".Cantidad.de.Operaciones",paste(nombre,".Volume",sep=""),".Monto.RSTctivo")
    ...
    accion=accion[c("Fecha", paste(nombre,".Open",sep=""), paste(nombre,".High",sep=""), 
                    ...
                    ".Cantidad.de.Operaciones", ".Monto.RSTctivo", "Tipo.de.operacion") ]
    ##REVISADO
  ...
  mercado=data.frame(
    ...
    numero_de_acciones=c(285642388),
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( RST$precio, mercado[mercado$`Acción`=="RST",4], min(RST$accion_usd$RST.Close), max(RST$accion_usd$RST.Close), mean(RST$accion_usd$RST.Close),
        as.numeric(var(RST$accion_usd$RST.Close)),  mercado[mercado$`Acción`=="RST",6], mercado[mercado$`Acción`=="RST",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(RST$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(RST$accion_bs)
