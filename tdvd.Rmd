---
title: "C.A. Nacional de Teléfonos de Venezuela (Reporte Grupo Interaval)"
...
<p align="center">
  <img width="320px" height="90px" src="tdvd.png">
</p>

La CoTDV.Dñía Anónima Nacional Teléfonos de Venezuela (CANTV) es una empresa estatal de telecomunicaciones venezolana. Sus servicios van desde telefonía, siendo su principal filial Movilnet que expande el área de telefonía celular, hasta servicios como venta de computadores, servicios de conexión a Internet dial-up o banda ancha (ABA, Acceso a Banda Ancha) y Televisión Satelital. Esta empresa cotiza en la Bolsa de Valores de Caracas bajo el símbolo "TDV.D" y hasta la fecha mantiene 26.121.595 acciones en circulación.
...
library(kableExtra)
  ...
  datos_accion <-function(accion, directorio, nombre){
    ...
    colnames(accion)<-c("Fecha","Tipo.de.operacion",paste(nombre,".Open",sep=""),paste(nombre,".Close",sep=""),
                        ...
                        ".Cantidad.de.Operaciones",paste(nombre,".Volume",sep=""),".Monto.TDV.Dctivo")
    ...
    accion=accion[c("Fecha", paste(nombre,".Open",sep=""), paste(nombre,".High",sep=""), 
                    ...
                    ".Cantidad.de.Operaciones", ".Monto.TDV.Dctivo", "Tipo.de.operacion") ]
    ##REVISADO
  ...
  TDV.D=datos_accion(accion,paste("C:/Users/analista04/Documents/R/grupointeraval.github.io//","TDV.D.txt",sep=""),"TDV.D")
  ...
  mercado=data.frame(
    ...
    numero_de_acciones=c(26121595),
    
    
    precio=c(TDV.D$precio)
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( TDV.D$precio, mercado[mercado$`Acción`=="TDV.D",4], min(TDV.D$accion_usd$TDV.D.Close), max(TDV.D$accion_usd$TDV.D.Close), mean(TDV.D$accion_usd$TDV.D.Close),
        as.numeric(var(TDV.D$accion_usd$TDV.D.Close)),  mercado[mercado$`Acción`=="TDV.D",6], mercado[mercado$`Acción`=="TDV.D",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(TDV.D$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(TDV.D$accion_bs)
