---
title: "Bolsa de Valores de Caracas (Reporte Grupo Interaval)"
...
El objetivo de la BVCC es facilitar la intermediacion de instrumentos financieros y difundir la informacion que requiere el mercado de manera competitiva, asegurando transparencia y eficacia dentro de un marco autorregulado y apegado a los principios legales y eticos; apoyandose para ello en el mejor recurso humano y en la solvencia de nuestros accionistas.
...
library(kableExtra)
  ...
  mercado=data.frame(
    ...
    numero_de_acciones=c(29250000),
    
    
    precio=c(BVCC$precio)
...
mercado=mercado[c("Acción","Número de acciones","Precio Bs.", "Precio $USD","Capitalización Bs","Capitalización $USD")]
 ...
 resumen=data.frame(
    ...
    valor=c( BVCC$precio, mercado[mercado$`Acción`=="BVCC",4], min(BVCC$accion_usd$BVCC.Close), max(BVCC$accion_usd$BVCC.Close), mean(BVCC$accion_usd$BVCC.Close),
        as.numeric(var(BVCC$accion_usd$BVCC.Close)), mercado[mercado$`Acción`=="BVCC",6], mercado[mercado$`Acción`=="BVCC",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(BVCC$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(BVCC$accion_bs)
