---
title: "Fondo de Valores Inmobiliarios (Reporte Grupo Interaval)"
...
<p align="center">
  <img width="380px" height="90px" src="fvi.png">
</p>

Esta sociedad tiene por objeto la adquisición y la explotación de bienes inmuebles o títulos valores. La compañía se orienta al diseño, ejecución y administración de proyectos inmobiliarios y de inversión, así como la promoción y constitución de toda clase de sociedades mercantiles dentro y fuera del territorio nacional, estando habilitada para celebrar toda clase de contratos civiles, mercantiles y administrativos. Esta empresa cotiza en la Bolsa de Valores de Caracas bajo los símbolos "FVI.A" y "FVI.B". Hasta la fecha mantiene 12.345.544.354 acciones en circulación.
...
library(kableExtra)
  ...
  datos_accion <-function(accion, directorio, nombre){
    ...
    colnames(accion)<-c("Fecha","Tipo.de.operacion",paste(nombre,".Open",sep=""),paste(nombre,".Close",sep=""),
                        ...
                        ".Cantidad.de.Operaciones",paste(nombre,".Volume",sep=""),".Monto.FVI.Activo")
    ...
    accion=accion[c("Fecha", paste(nombre,".Open",sep=""), paste(nombre,".High",sep=""), 
                    ...
                    ".Cantidad.de.Operaciones", ".Monto.FVI.Activo", "Tipo.de.operacion") ]
    ##REVISADO
    ...
    fecha_split=as.Date("2020-10-16", format="%Y-%m-%d")
  ...
  FVI.A=datos_accion(accion,paste("C:/Users/analista04/Documents/R/grupointeraval.github.io//","FVI.A.txt",sep=""),"FVI.A")
  ...
  mercado=data.frame(
    ...
    numero_de_acciones=c(18877764),
    
    
    precio=c(FVI.A$precio)
...
resumen=data.frame(
    ...
    valor=c( FVI.A$precio, mercado[mercado$`Acción`=="FVI.A",4], min(FVI.A$accion_usd$FVI.A.Close), max(FVI.A$accion_usd$FVI.A.Close), mean(FVI.A$accion_usd$FVI.A.Close),
        as.numeric(var(FVI.A$accion_usd$FVI.A.Close)), mercado[mercado$`Acción`=="FVI.A",6], mercado[mercado$`Acción`=="FVI.A",2])
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel=as.data.frame(FVI.A$accion_usd)
excel[,4] <- NULL
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
excel2=as.data.frame(FVI.A$accion_bs)
